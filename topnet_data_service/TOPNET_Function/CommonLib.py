...
def watershed_delineation(DEM_Raster,Outlet_shapefile,Src_threshold,Min_threshold,Max_threshold,Number_threshold,
                          output_pointoutletshapefile,output_watershedfile,output_treefile,output_coordfile,output_streamnetfile,output_slopareafile,output_distancefile):
    ...
    # Prasanna: some error were returned when the path was not changed. So I added the code to change the path
    working_dir = os.path.split(output_watershedfile)[0]
...
def PITREMOVE(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "pitremove"));
    commands.append("-z")
    commands.append(os.path.join(In_Out_dir, Input_Dem + ".tif"));
    commands.append("-fel")
...
def D8FLOWDIRECTIONS(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "d8flowdir"));
    commands.append("-p")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "p.tif"));
    commands.append("-sd8")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "sd8.tif"));
    commands.append("-fel")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "fel.tif"))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    return fused_command
...
def D8CONTRIBUTING_AREA(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "aread8"));
    commands.append("-p")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "p.tif"));
    commands.append("-ad8")
...
def DINFFLOWDIRECTIONS(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "dinfflowdir"));
    commands.append("-ang")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ang.tif"));
    commands.append("-slp")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "slp.tif"));
    commands.append("-fel")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "fel.tif"))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    return fused_command
...
def DINFCONTRIBUTINGAREA(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "areadinf"));
    commands.append("-ang")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ang.tif"));
    commands.append("-sca")
...
def THRESHOLD(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, Src_threshold):
    ...
    commands.append(os.path.join(TauDEM_dir, "threshold"));
    commands.append("-ssa")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ad8.tif"));
    commands.append("-src")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "src.tif"));
    commands.append("-thresh");
    commands.append(str(Src_threshold))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    return fused_command
...
def MOVEOUTLETTOSTREAMS(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, Outlet, output_pointoutletshapefile):
    ...
    commands.append(os.path.join(In_Out_dir, Input_Dem + "src.tif"));
    commands.append("-o");
    commands.append(os.path.join((In_Out_dir + Outlet), Outlet))
    commands.append("-om");
...
def PEUKERDOUGLAS(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem):
    ...
    commands.append(os.path.join(TauDEM_dir, "peukerdouglas"));
    commands.append("-fel")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "fel.tif"));
    commands.append("-ss")
...
def ACCUMULATING_STREAM_SOURCE(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, output_pointoutletshapefile):
    ...
    commands.append(os.path.join(In_Out_dir, output_pointoutletshapefile));
    commands.append("-ad8")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ssa.tif"));
    commands.append("-wg")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ss.tif"))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    return fused_command
...
def DROP_ANALYSIS(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, Min_threshold, Max_threshold, Number_threshold,
                  output_pointoutletshapefile):
    commands = []
    # commands.append(os.path.join(MPI_dir,"mpirun"));commands.append("-np");commands.append(str(np))
    commands.append(os.path.join(TauDEM_dir, "dropanalysis"));
    commands.append("-p")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "p.tif"));
    commands.append("-fel")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "fel.tif"));
    ...
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ssa.tif"));
    commands.append("-drp")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "drp.txt"));
    commands.append("-o")
    commands.append(os.path.join(In_Out_dir, output_pointoutletshapefile));
    commands.append("-par")
    commands.append(str(Min_threshold));
    commands.append(str(Max_threshold));
    commands.append(str(Number_threshold));
...
def STREAM_SOURCE(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, thershold):
    ...
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ssa.tif"));
    commands.append("-src")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "src2.tif"));
    commands.append("-thresh")
...
def StreamNet(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, output_pointoutletshapefile, output_watershedfile,
              output_treefile, output_coordfile, output_streamnetfile):
    commands = []
    # commands.append(os.path.join(MPI_dir,"mpirun"));commands.append("-np");commands.append(str(np))

    commands.append(os.path.join(TauDEM_dir, "streamnet"));
    commands.append("-fel")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "fel.tif"));
    commands.append("-p")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "p.tif"));
    ...
    commands.append(os.path.join(In_Out_dir, Input_Dem + "src2.tif"));
    commands.append("-ord")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ord2.tif"));
    commands.append("-tree")
    commands.append(os.path.join(In_Out_dir, output_treefile));
    commands.append("-coord")
    commands.append(os.path.join(In_Out_dir, output_coordfile));
    commands.append("-net")
    commands.append(os.path.join(In_Out_dir, output_streamnetfile));
    commands.append("-w")
    commands.append(os.path.join(In_Out_dir, output_watershedfile));
    commands.append("-o")
    commands.append(os.path.join(In_Out_dir, output_pointoutletshapefile))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    return fused_command
...
def WETNESS_INDEX(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, output_slopareafile):
    ...
    commands.append(os.path.join(TauDEM_dir, "slopearearatio"));
    commands.append("-slp")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "sd8.tif"));
    commands.append("-sca")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "ad8.tif"));
...
def DISTANCE_DOWNSTREAM(MPI_dir, np, TauDEM_dir, In_Out_dir, Input_Dem, output_distancefile):
    ...
    commands.append(os.path.join(TauDEM_dir, "d8hdisttostrm"));
    commands.append("-p")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "p.tif"));
    commands.append("-src")
    commands.append(os.path.join(In_Out_dir, Input_Dem + "src.tif"));
...
def REACH_LINK(DEM_Raster, Watershed_Raster, treefile, coordfile, output_reachfile, output_nodefile,
               output_reachareafile, output_rchpropertiesfile):
    head, tail = os.path.split(str(DEM_Raster))
    In_Out_dir = str(head)
    base_name = os.path.basename(DEM_Raster)
    Input_Dem = os.path.splitext(base_name)[0]
    ...
    commands.append("-me");
    commands.append(Watershed_Raster)
    commands.append("-p");
    commands.append(os.path.join(In_Out_dir, Input_Dem + 'p.tif'))
    commands.append("-tree");
    commands.append(treefile)
    commands.append("-coord");
    commands.append(coordfile)
    commands.append("-rchlink");
    commands.append(os.path.join(In_Out_dir, output_reachfile))
    commands.append("-nodelink");
    commands.append(os.path.join(In_Out_dir, output_nodefile))
    commands.append("-nc");
    commands.append(os.path.join(In_Out_dir, Input_Dem + "nc.tif"))
    commands.append("-dc");
    commands.append(os.path.join(In_Out_dir, Input_Dem + "dc.tif"))
    commands.append("-rca");
    commands.append(os.path.join(In_Out_dir, output_reachareafile))
    commands.append("-rcp");
    commands.append(os.path.join(In_Out_dir, output_rchpropertiesfile))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    # return fused_command
...
def DISTANCE_DISTRIBUTION(Watershed_Raster, SaR_Raster, Dist_Raster, output_distributionfile):
    ...
    commands.append(Watershed_Raster)
    commands.append("-twi");
    commands.append(SaR_Raster)
    commands.append("-dis");
    commands.append(Dist_Raster)
    commands.append("-dists");
    commands.append(os.path.join(In_Out_dir, output_distributionfile))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    os.system(fused_command)
    return {'success': 'True', 'message': 'download daymetdata successful'}
...
def download_Soil_Data(Watershed_Raster, output_f_file, output_k_file, output_dth1_file, output_dth2_file,
                       output_psif_file, output_sd_file, output_tran_file):
    head, tail = os.path.split(str(Watershed_Raster))
    ...
    commands.append(Soil_script);
    commands.append(str(wateshed_Dir))
    commands.append(str(tails));
    commands.append(str(output_f_file))
    commands.append(str(output_k_file));
    commands.append(str(output_dth1_file))
    commands.append(str(output_dth2_file));
    commands.append(str(output_psif_file))
    commands.append(str(output_sd_file));
    commands.append(str(output_tran_file))
...
def daymet_download(Watershed_Raster,Start_Year,End_Year,output_rainfile,output_temperaturefile,output_cliparfile,output_gagefile):
    head,tail=os.path.split(str(Watershed_Raster))
    watershed_dir =str(head) # ORIGINALLY
    wateshed_name = str(tail)

    gage_fname = os.path.split(str(output_gagefile))[-1]
    os.chdir(watershed_dir)



    daymet_download_script= os.path.join(R_Code_Path,'daymet_download.r')
    commands=[]
    commands.append("Rscript");commands.append(daymet_download_script);
    commands.append(str(watershed_dir))
    commands.append(str(wateshed_name));
    commands.append(str(Start_Year));
    commands.append(str(End_Year))
    commands.append(str(gage_fname)) #output_gagefile
    fused_command1 = ''.join(['"%s" ' % c for c in commands])
    os.system(fused_command1)
    print ('daymet_download.R Completed')
    ...
    print ('create_tmaxtmintdew.R Completed')
    ...
    print ('create_clipar.R Completed')
    return {'success': 'True', 'message': 'download daymetdata successful'}
...
def Create_Parspcfile(Watershed_Raster, output_parspcfile):
    ...
    commands = []
    commands.append("Rscript");
    commands.append(parspc_script)
    commands.append(str(watershed_dir));
    commands.append(str(wateshed_name));
    commands.append(str(output_parspcfile));
    fused_command = ''.join(['"%s" ' % c for c in commands])
    os.system(fused_command)
    return {'success': 'True', 'message': 'download LULC data  successful'}
...
def create_latlonfromxy(Watershed_Raster, output_latlonfromxyfile):
    ...
    commands = []
    commands.append("Rscript");
    commands.append(latlonxy_script)
    commands.append(str(watershed_dir));
    commands.append(str(wateshed_name))
...
def Create_rain_weight(Watershed_Raster, Rain_gauge_shapefile, annual_rainfile, nodelink_file, output_rainweightfile):
    ...
    commands.append("-w");
    commands.append(os.path.join(watershed_dir, wateshed_name))
    commands.append("-rg");
    commands.append(os.path.join(Rain_GaugeDir, tail_rg))
    commands.append("-ar");
    commands.append(os.path.join(watershed_dir, tail_an))
    commands.append("-tri");
    commands.append(os.path.join(watershed_dir, "triout.tif"))
    commands.append("-wt");
    commands.append(os.path.join(watershed_dir, "weights.txt"))
    fused_command1 = ''.join(['"%s" ' % c for c in commands])
    os.system(fused_command1)
    ...
    commands = []
    commands.append("Rscript");
    commands.append(format_rainweight_script)
    commands.append(str(watershed_dir));
    commands.append(str(tail_nd))
...
def download_streamflow(USGS_gage, Start_Year, End_Year, output_streamflow):
    ...
    commands.append(str(USGS_gage));
    commands.append(str(start));
    commands.append(str(end))
    commands.append(str(Output_Dir));
    commands.append(str(output_streamflow))
    fused_command = ''.join(['"%s" ' % c for c in commands])
    os.system(fused_command)
...
def create_basin_param(watershed_shapefile, lancover_raster, lutluc, lutkc, paramfile, nodelinkfile, output_basinfile):
    ...
    for i in range(0, data_length):
        ...
        if (soil_pro_file == 0):
            ...
            df2 = pd.DataFrame(
                reclassify_and_stats_raster(str(input_datalist[2][i]), unique_values, np.array(lulc_table[col_ind]),
                                            watershed_shapefile))
            ...
            col_ind = int(input_datalist[4][i])
            df3 = pd.DataFrame(
                reclassify_and_stats_raster(str(input_datalist[2][i]), unique_values, np.array(lukc_table[col_ind]),
                                            watershed_shapefile))
    ...
    # print(df)
    nodfile_data = pd.read_csv(nodelinkfile,
                               dtype={'NodeId': np.int, 'DownNodeId': np.int, 'DrainId': np.int, 'ProjNodeId': np.int,
                                      'DOutFlag': np.int, 'ReachId': np.int, 'Area': np.float64,
                                      'AreaTotal': np.float64, 'X': np.float64, 'Y': np.float64})
    nd_df = pd.DataFrame(nodfile_data)
    nd_df.columns = ['NodeId', 'DownNodeId', 'DrainId', 'ProjNodeId', 'DOutFlag', 'ReachId', 'Area', 'AreaTotal', 'X',
                     'Y']
    nd_df['Area'] = nd_df['Area'] * 1000000
    # print (nd_df)
    # CatchID,DownCatchID,DrainID,NodeId,Reach_number,Outlet_X,Outlet_Y,direct_area,

    df.columns = ['DrainId', 'f', 'ko', 'dth1', 'dth2', 'soildepth', 'c', 'psif', 'chv', 'cc', 'cr', 'albedo',
                  'LapseRate', 'AverageElevation', 'ImperviousFraction', 'TileDrainedFraction', 'DitchDrainedFraction',
                  'TileCoeff', 'DitchCoef', 'IrrigatedFraction', 'SprinklerFractionofIrrigation',
                  'IrrigationEfficiency', 'D_Thres', 'Z_Max', 'D_Goal',
                  'kc_1', 'kc_2', 'kc_3', 'kc_4', 'kc_5', 'kc_6', 'kc_7', 'kc_8', 'kc_9', 'kc_10', 'kc_11', 'kc_12',
                  'Transmissivity', 'FractionForest']
    nd_df.columns = ['NodeId', 'DownNodeId', 'DrainId', 'ProjNodeId', 'DOutFlag', 'ReachId', 'Area', 'AreaTotal', 'X',
                     'Y']
...
def BASIN_PARAM(DEM_Raster, Watershed_Raster, f_raster, k_raster, dth1_raster, dth2_raster, sd_raster, psif_raster,
                tran_raster, lulc_raster,
    ...
    # print('test2done')
    create_basin_param('final.shp', lulc_raster, lutlc, lutkc, parameter_specficationfile, nodelinksfile,
                       output_basinfile)
...
# Prasanna added this
def subsetprism( output_raster=None, xmin=None, ymax=None, xmax=None, ymin=None):
    ...
    cmdString = "gdal_translate" + " " + "-projwin" + " " + str(xmin) + " " + str(ymax) + " " \
                + str(xmax) + " " + str(ymin) + " " + '/home/ahmet/hydosdata/PRISM_annual/PRISM_ppt_30yr_normal_800mM2_annual_bil.bil' + " " + output_raster

    return call_subprocess(cmdString, 'get prism raster subset')
...
def add_filed_existing_shapefile( inputfile,arr):
   # Read in our existing shapefile
...
def download_soil_data_for_pytopkapi(Watershed_Raster, output_f_file='f.tif', output_k_file='ko.tif', output_dth1_file='dth1.tif', output_dth2_file='dth2.tif',
    ...
    Soil_script = os.path.join(R_Code_Path, 'Extract_Soil_Data_pytopkapi.r')
    heads, tails = os.path.split(str(soil_output_file))
    commands = []
    commands.append("Rscript");
    commands.append(Soil_script);
    ...
    commands.append(str(output_bubbling_pressure_file))
    commands.append(str(output_pore_size_distribution_file))
    commands.append(str(output_residual_soil_moisture_file))
    commands.append(str(output_saturated_soil_moisture_file))
    commands.append(str(output_ksat_rawls_file))
