...
def read_raster(rast_fname, file_format='GTiff'):
    ...
    if file_format != 'GTiff':
        ...
        # these two lines added, bcause there was some problem with nodata values
        nodata = dset.GetRasterBand(1).GetNoDataValue()
...
def generate_param_file(ini_fname, isolated_cells=False):
    ...
    flowdir[mask != 1] = -9999  # added by Prasanna, to make sure pytopkapi ignores fdr outside mask
    ...
    # PRASANNNA added the two lines below, commented two lines above
    channel_network[channel_network != 1] = 0
